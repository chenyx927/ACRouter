...
class_getInstanceMethod([KenModule class], @selector(module_##__SELECTORSTRING__)));
        ...
        //iphone xs 可能处理器的原因，导致方法的先后有顺序处理有问题，固放弃(application:willFinishLaunchingWithOptions:)
//        SwizzleDelegateMethod(application:willFinishLaunchingWithOptions:);
        ...
        SwizzleDelegateMethod(application:handleOpenURL:);
        SwizzleDelegateMethod(application:openURL:sourceApplication:annotation:);
...
- (BOOL)module_application:(UIApplication *)application openURL:(NSURL *)url
    ...
    BOOL result = YES;
    if (imp1 != imp2) {
        result = ((BOOL (*)(id, SEL, id, id, id))(void *)objc_msgSend)(self, selector, application, url, options);
    }

    BOOL (*typed_msgSend)(id, SEL, id, id, id) = (void *)objc_msgSend;
    for (Class cls in KenModuleClasses) {
        if ([cls respondsToSelector:_cmd]) {
            result = typed_msgSend(cls, _cmd, application, url, options);
        }
    }
    return result;
}
...
- (BOOL)module_application:(UIApplication *)application handleOpenURL:(NSURL *)url {
    KenReturnMethodSend(application, url);
}

- (BOOL)module_application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(nullable NSString *)sourceApplication annotation:(id)annotation {
    SEL selector = NSSelectorFromString([NSString stringWithFormat:@"module_%@", NSStringFromSelector(_cmd)]);
    ...
    for (Class cls in KenModuleClasses) {
        if ([cls respondsToSelector:_cmd]) {
            result = typed_msgSend(cls, _cmd, application, url, sourceApplication, annotation);
        }
    }
    return result;
}
